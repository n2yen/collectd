...
Selects one named data source from the matched type, such as C<rx> from
C<if_octets>. The name must match a data source in L<types.db(5)> for the
configured B<Type>. This option is required when B<Type> contains more than
one data source.
...
When B<DataSource> selects one value from a multi-data-source type, B<SetType>
must name a single-value type used for the aggregated output. Its data source
must have the same type (C<COUNTER>, C<DERIVE>, C<GAUGE>, or C<ABSOLUTE>) as
the selected input data source.
